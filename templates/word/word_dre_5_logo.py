# templates/word/word_dre_5_logo.py (Updated with optimized cell heights)
import os
import json
import shutil
...
def _set_dre5_column_widths(table):
    """Set table to use exact column widths for 9-column dressage layout - ORIGINAL STREAM WIDTHS"""
    tbl = table._tbl
    ...
    tblGrid = OxmlElement('w:tblGrid')
    # BACK TO ORIGINAL WORKING STREAM WIDTHS aber mit breiterer KoNr: Start | KoNr. (breiter) | Reiter/Pferd | E | H | C | M | B | Total
    original_stream_widths_twips = [450, 540, 6500, 600, 600, 600, 600, 600, 800]  # KoNr von 288 auf 540 erweitert
    
    for width in original_stream_widths_twips:
        gridCol = OxmlElement('w:gridCol')
        gridCol.set(qn('w:w'), str(width))
        tblGrid.append(gridCol)
    ...
    for row in tbl.tr_lst:
        for i, cell in enumerate(row.tc_lst):
            if i < len(original_stream_widths_twips):
                tcPr = cell.tcPr
                if tcPr is None:
                    tcPr = OxmlElement("w:tcPr")
                    cell.insert(0, tcPr)
                ...
                tcW = OxmlElement('w:tcW')
                tcW.set(qn('w:w'), str(original_stream_widths_twips[i]))
                tcW.set(qn('w:type'), 'dxa')
                tcPr.append(tcW)

def _get_ordered_judge_positions_main_table(judges):
    """
    Bestimmt die Richter für die Haupttabelle: E H C M B in fester Reihenfolge, 
    maximal 5 Spalten - nur die tatsächlich vorhandenen Positionen
    """
    if not judges:
        return ["E", "H", "C", "M", "B"]
    ...
    # KORRIGIERT: Nur die tatsächlich vorhandenen Positionen verwenden
    # Wenn keine Richter gefunden wurden, alle 5 Positionen anzeigen
    if not ordered_positions:
        ordered_positions = ["E", "H", "C", "M", "B"]
    
    # Immer auf genau 5 Spalten auffüllen, aber mit leeren Strings für fehlende Positionen
    while len(ordered_positions) < 5:
        ordered_positions.append("")
    
    return ordered_positions[:5]

def _get_judge_data_for_display(judges):
    """
    Prozessiert Richter für die Anzeige - nur E H C M B Positionen
    """
    pos_map = {
        0: "E", 1: "H", 2: "C", 3: "M", 4: "B", 5: "K", 6: "V", 
        7: "S", 8: "R", 9: "P", 10: "F", 11: "A",
    ...
    # Return judges in E H C M B order
    result = []
    for pos in dressage_positions:
        if pos in judges_by_position:
            judge_data = judges_by_position[pos].copy()
            judge_data["pos_label"] = pos
            result.append(judge_data)
    
    return result

def render(starterlist: dict, filename: str):
    """Creates a Word document with 9-column dressage layout and prominent logo display with optimized cell heights"""
    print("WORD DRE 5 LOGO DEBUG: Starting render function with optimized cell heights")
    ...
    # Use the fixed Word template as base - KORREKT WIE URSPRÜNGLICH
    ...
    print("WORD DRE 5 LOGO DEBUG: Loading document from template")
    ...
    # Get judge positions for table headers
    ...
    print(f"WORD DEBUG: Judge positions returned: {judge_positions}")
    ...
    # Create table DIREKT nach kompaktem Header
    print("WORD DEBUG: Creating table after compact header")
    ...
    for group_num in sorted(starters_by_group.keys()):
        ...
        for starter_idx, starter in enumerate(group_starters):
            ...
            if rider_name:
                content_parts.append(rider_name)  # Normaler Case (kein .upper())
            if club_or_country:
                content_parts.append(("club", club_or_country))  # Markiert als Verein
            ...
            if horses:
                ...
                # Pferdename und Zuchtgebiet als eine Zeile kombinieren
                horse_line_parts = []
                if horse_name:
                    horse_line_parts.append((horse_name.upper(), True))  # (Text, Bold)
                if studbook:
            ...
            for i, part in enumerate(content_parts):
                ...
                if isinstance(part, list):  # Liste von (Text, Bold) Tupeln
                    for text, is_bold in part:
                        ...
                        run.font.size = Pt(9)
                elif isinstance(part, tuple) and part[0] == "club":
            ...
            # Column 8: Total column
            p8 = row_cells[8].paragraphs[0]
            p8.clear()
            p8.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
            _optimize_paragraph_spacing(p8)
            if is_hors_concours:
                ak_run = p8.add_run("AK")
    ...
    if judges:
        ...
        richter_header = f"Aufgabe: {test_name}" if test_name else ""
        if richter_header:
            p_aufgabe = doc.add_paragraph()
            run_aufgabe = p_aufgabe.add_run(richter_header)
            run_aufgabe.font.bold = True
            run_aufgabe.font.size = Pt(11)
            p_aufgabe.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT

        
        # Get judge data for display
        judge_display_data = _get_judge_data_for_display(judges)
        ...
        if judge_display_data:
            ...
            for i, header_text in enumerate(headers):
                ...
                run.font.bold = True
                run.font.size = Pt(9)
                # "Richter" linksbündig, "Pos" zentriert
    ...
    try:
        ...
        print(f"WORD DRE 5 LOGO DEBUG: Document saved to {output_path}")
        return output_path
    except Exception as e:
        ...
        try:
            ...
            print(f"WORD DRE 5 LOGO DEBUG: Document saved to alternative location: {alt_output_path}")
