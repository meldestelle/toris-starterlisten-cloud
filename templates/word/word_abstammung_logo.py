...
def render(starterlist: dict, filename: str):
    ...
    # Seitenränder bereits oben gesetzt
    ...
    for group_num in sorted(starters_by_group.keys()):
        ...
        for starter_idx, starter in enumerate(group_starters):
            ...
            if rider_name:
                run = p2.add_run(rider_name)
                run.font.bold = True
                run.font.size = Pt(9)
            if club_or_country:
                p2.add_run("\n")
                run = p2.add_run(club_or_country)
                run.font.size = Pt(7)
                run.font.bold = False
    ...
    # SPONSORENLEISTE IM FUß JEDER SEITE
    if show_sponsor_bar:
                ...
                for section in doc.sections:
                    ...
                    if show_banner and section.different_first_page_header_footer:
                        try:
                            ...
                            _fpfp.clear()
                            _fpfp.add_run().add_picture(sponsor_logo_path, width=Inches(7.5))
                            _fpfp.paragraph_format.space_after = Pt(0)
                            _fpfp.paragraph_format.space_before = Pt(0)
                        except: pass
                print(f"WORD ABSTAMMUNG LOGO DEBUG: Sponsorenleiste im Footer hinzugefügt")
            except Exception as e:
