...
WEEKDAY_MAP = {}  # Use English directly

MONTH_MAP = {}  # Use English directly


# Mapping für Geschlecht - ENGLISH
SEX_MAP = {
    "MARE": "mare",
    "GELDING": "gelding", 
    "STALLION": "stallion",
...
def get_country_name(ioc_code):
    ...
    names = {
        ...
        "SKN": "St. Kitts and Nevis", "SLE": "Sierra Leone", "SLO": "Slovenia",
...
def render(starterlist: dict, filename: str, logo_max_width_cm: float = 5.0):
    ...
    if needs_custom_margins:
        ...
        if show_sponsor_bar:
            ...
            min_bottom_mm = (7*mm + sponsor_height + 1*mm) / mm
        else:
            min_bottom_mm = 10
        top_margin_front = (spacing_top_cm * 10) if sponsor_top or not show_header else 0.8 * 10
        top_margin_back = 0.8 * 10
        ...
        bottom_margin = 7*mm + sponsor_height + 1*mm if show_sponsor_bar else 8*mm
    ...
    if show_header:
        ...
        comp_title_text = comp.get("title") or starterlist.get("competitionTitle") or ""
    ...
    # --- Richter ÜBER der Tabelle ---
    judges = comp.get("judges") or starterlist.get("judges") or []
    if judges:
        # Nur Richter mit Position "C" anzeigen (keine Sonder-Positionen)
        c_judges = [j for j in judges if j.get("position") == "C" and j.get("name")]
        if c_judges:
            judge_names = [j.get("name", "") for j in c_judges]
            judges_text = " & ".join(judge_names)
            elements.append(Paragraph(f"<b>Judges:</b> {judges_text}", style_sub))
            elements.append(Spacer(1, 3))

    data_texts = []
    meta = []
    # Header Zeile 1: 8 Spalten, "Stechen" spannt über 4 Spalten (4-7)
    data_texts.append([
        "<b># / CNO</b>", 
        "<b>Athlete / Horse</b><br/><font size=7>Age / Colour / Sex / Sire x Sire Dam / Owner / Breeder</font>", 
        "<b><font size=6>Nat</font></b>", 
        "<b>Rnd.</b>",
        "<b>Jump Off</b>",
        "", "", ""  # Diese 3 merged mit Stechen
    ])
    meta.append({"type":"header_row1"})
    
    # Header Zeile 2: Die Nummern unter Stechen
    data_texts.append([
        "", "", "", "",
        "<b>1</b>", "<b>2</b>", "<b>3</b>", "<b>4</b>"
    ])
    meta.append({"type":"header_row2"})
    ...
    for s in starters:
        ...
        if starter_group is not None and starter_group > 0 and starter_group != current_group:
            ...
            data_texts.append([group_text, "", "", "", "", "", "", ""])  # 8 Spalten
        ...
        data_texts.append([start_knr_table, combined_content, nat_cell, "", "", "", "", ""])  # 8 Spalten
    ...
    col_uml = 12*mm  # Uml.
    col_stechen = [10*mm] * 4  # 4 Stechen-Spalten  
    col2 = page_width - col1 - col_nat - col_uml - sum(col_stechen)  # Reiter+Pferd (Rest)
    col_widths = [col1, col2, col_nat, col_uml] + col_stechen  # 8 Spalten total
    ...
    for i, row in enumerate(data_texts):
        ...
        m = meta[i]
        if m["type"] == "header_row1":
            # Header Zeile 1 - mit "Stechen" über 4 Spalten
            table_rows.append([
        ...
        elif m["type"] == "header_row2":
            # Header Zeile 2 - die Nummern 1 2 3 4
            table_rows.append([
            ...
            # Fallback für alten header type
            table_rows.append([
                Paragraph(row[0], style_hdr), Paragraph(row[1], style_hdr_left),
                Paragraph(row[2], style_hdr), Paragraph(row[3], style_hdr),
                Paragraph(row[4], style_hdr), Paragraph(row[5], style_hdr),
            ...
            table_rows.append([
                ...
                maybe_strike(row[7], style_pos)
            ])

    t = Table(table_rows, colWidths=col_widths, repeatRows=2)
    ts = TableStyle([
        ...
        ("SPAN", (4,0), (7,0)),  # "Stechen" Header über 4 Spalten (4,5,6,7)
        ("SPAN", (0,0), (0,1)),  # # / KNr über beide Header-Zeilen
        ("SPAN", (1,0), (1,1)),  # Reiter/Pferd über beide Header-Zeilen
        ("SPAN", (2,0), (2,1)),  # Nat über beide Header-Zeilen
        ("SPAN", (3,0), (3,1)),  # Uml. über beide Header-Zeilen
    ])
