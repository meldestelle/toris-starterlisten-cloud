    ...
    def draw_banner(self):
        ...
        if self.banner_path and os.path.exists(self.banner_path):
            try:
                ...
                self.drawImage(self.banner_path, x, y, width=self.banner_width, height=self.banner_height, preserveAspectRatio=True, mask='auto')
            except:
                pass
    def draw_footer(self):
        ...
        if os.path.exists(sponsor_path):
            try:
                ...
                x = (A4[0] - img_width) / 2
                y = 4*mm
                self.drawImage(sponsor_path, x, y, width=img_width, height=img_height, preserveAspectRatio=True, mask='auto')
            except:
                pass
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    for s in starters:
        ...
        if horse:
            ...
            if owner:
                horse_html += f"<br/><font size=6.5><i>Besitzer: {owner}</i></font>"
            # Züchter-Zeile entfernt für pdf_nat_spr
