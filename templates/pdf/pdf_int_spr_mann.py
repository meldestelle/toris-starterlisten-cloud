# -*- coding: utf-8 -*-
# templates/pdf/pdf_int_spr_mann.py - International Springen Mannschaft, sortiert nach Team dann Startnummer
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    style_pos = ParagraphStyle('Pos', fontSize=8, leading=7, fontName='Helvetica', alignment=TA_CENTER)
    style_rider = ParagraphStyle('Rider', fontSize=9, leading=9, fontName='Helvetica', alignment=TA_LEFT)

    style_horse = ParagraphStyle('Horse', fontSize=9, leading=9, fontName='Helvetica', alignment=TA_LEFT)
    ...
    # Tabelle - International Mannschaftsprüfung
    ...
    for s in starters_sorted:
        ...
        if team_no != current_team:
            ...
            if team_no is not None:
                header_text = f"Team {team_no}: {team_name}" if team_name else f"Team {team_no}"
            else:
                header_text = "No Team"
        ...
        if horse:
            ...
            # Pferdename fett, Studbook klein dahinter mit " - "
            ...
            # Studbook nicht mehr in details - ist jetzt hinter Namen
        ...
        # Owner separat holen
        owner = horse.get("owner", "") if horse else ""
    ...
    # Hilfsfunktion: eine Tabellenzeile bauen
    def build_row(row, m):
        if m["type"] == "header":
            return [
    ...
    # Pro Team eine KeepTogether-Tabelle
    while i < len(meta):
        m = meta[i]
        if m["type"] == "team":
            seg_rows = []
