# -*- coding: utf-8 -*-
# templates/pdf/pdf_dre_derby_int.py - Derby format: 2 Semifinals + Final (English)
#
# Structure:
#   Starters 1 vs 2  → 1st Semifinal
#   Starters 3 vs 4  → 2nd Semifinal
#   Starters 5 vs 6  → Final (athlete names not shown, not yet determined)
#
# Pop-up input:  Start of Competition (before starter 1) + Final time (before starter 5)
# Judges row:    Inline format from pdf_int
# Starting order: Column format from pdf_dre_5_int
...
# ---------------------------------------------------------------------------
def _ask_times():
    """Öffnet ein Tkinter-Fenster und fragt Prüfungsbeginn und Finale-Uhrzeit ab."""
    root = tk.Tk()
    root.withdraw()
    # Vordergrund erzwingen (verhindert dass Dialoge hinter Browser erscheinen)
    root.attributes('-topmost', True)
    root.lift()
    root.after(10, lambda: root.attributes('-topmost', False))

    begin_time = simpledialog.askstring(
        "Derby – Start of Competition",
        "Time – Start of Competition (e.g. 10:00):",
        parent=root
    ) or ""

    final_time = simpledialog.askstring(
        "Derby – Final",
        "Time – Final (e.g. 11:30):",
        parent=root
    ) or ""

    root.destroy()
    return begin_time.strip(), final_time.strip()
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    begin_label = f"Start of Competition {begin_time}".strip() if begin_time else "Start of Competition"
    ...
    for idx, s in enumerate(starters[:6]):
        ...
        if star_num == 5:
            finale_label = f"Final {final_time}".strip() if final_time else "Final"
        ...
        if star_num >= 5:
            ...
            combined_content = "Athlete: _____________________________________________<br/>" + ("<br/>".join(horse_parts) if horse_parts else "")
    ...
    print(f"PDF Derby INT Cloud: {filename}")
