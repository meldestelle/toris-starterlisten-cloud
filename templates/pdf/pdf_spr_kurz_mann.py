# -*- coding: utf-8 -*-
# templates/pdf/pdf_spr_kurz_mann.py - wie pdf_spr_kurz, aber Vereinsname durch Mannschaftsname ersetzt
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    for s in starters:
        ...
        # Reiter-HTML: Name + Mannschaft/Land darunter
        athlete_html = f"<b>{athlete_name}</b>" if athlete_name else ""
        
        # Ausländer zeigen Land nur wenn Mannschaftsname leer ODER "Gastlizenz GER"
        if nationality and nationality.upper() != "GER":
            # Ausländer
            if not team_name or team_name.strip() == "" or team_name.strip().upper() == "GASTLIZENZ GER":
                # Kein Mannschaftsname oder Gastlizenz → Land ausgeschrieben anzeigen
                country_full = get_country_name(nationality)
                if country_full:
                    athlete_html += f"<br/><font size=7>{country_full}</font>"
            else:
                # Hat einen Mannschaftsnamen → anzeigen
                athlete_html += f"<br/><font size=7>{team_name}</font>"
        elif team_name:
            # Deutsche: Mannschaftsname
            athlete_html += f"<br/><font size=7>{team_name}</font>"
    ...
    ts = TableStyle([
        ...
        # Kompakte Abstände wie andere Templates
