# -*- coding: utf-8 -*-
# templates/pdf/pdf_int_spr_banner.py - International Springen Banner Design - mit printOptions-Unterstützung
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    # Header
    data_texts.append(["#", "CNO", "Horse", "Athlete<br/><font size=7>Owner</font>", "Nat."])
    ...
    for s in starters:
        ...
        # Reiter-HTML: Name + Owner darunter
        athlete_html = f"<b>{athlete_name}</b>" if athlete_name else ""
        if owner:
            athlete_html += f"<br/><font size=7>{owner}</font>"
    ...
    ts = TableStyle([
        ...
        # Kompakte Zeilen mit minimalem Padding
        ("TOPPADDING", (0,0), (-1,-1), 1),
    ...
    if show_banner or show_sponsor_bar:
        ...
        doc.build(elements)
    print(f"PDF INT BANNER: {filename}")
