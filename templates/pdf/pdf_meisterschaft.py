# -*- coding: utf-8 -*-
# templates/pdf/pdf_meisterschaft.py - Meisterschaftsliste - basierend auf pdf_std_spr_2ph
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    style_rider = ParagraphStyle('Rider', fontSize=9, leading=8.5, fontName='Helvetica', alignment=TA_LEFT)

    style_horse = ParagraphStyle('Horse', fontSize=9, leading=8.5, fontName='Helvetica', alignment=TA_LEFT)
    ...
    data_texts.append(["#", "KNR", "Pferd", "Reiter", "Nat.", "Meisterschaft", "Fehler", "Zeit"])
    ...
    for s in starters:
        ...
        # Meisterschaft-Lookup: Summe aus Excel via cno (KNr.)
        # cno normalisieren: ggf. Float-String "565.0" → "565", Leerzeichen entfernen
        _ms_data = print_options.get("meisterschaft_data", {})
        _cno_lookup = cno.strip()
        try:
            _cno_lookup = str(int(float(_cno_lookup)))
        except:
            pass
        ms_summe = _ms_data.get(_cno_lookup, None)
        if ms_summe is not None:
            try:
                ms_display = f"{float(ms_summe):.2f}"
            except:
                ms_display = str(ms_summe)
        else:
            ms_display = ""
        if _ms_data:
            print(f"DEBUG Meisterschaft: cno={repr(cno)} → lookup={repr(_cno_lookup)} → summe={repr(ms_summe)}")
        data_texts.append([nr_display, cno, horse_html, athlete_html, nat_cell, ms_display, "", ""])
    ...
    # Spaltenbreiten
    # 8 Spalten: #, KNR, Pferd, Reiter, Nat., Meisterschaft, Fehler, Zeit
    result_col = 16*mm
    athlete_col = 48*mm
    ...
    if show_banner or show_sponsor_bar:
        ...
        doc.build(elements)
    print(f"PDF MEISTERSCHAFT: {filename}")
