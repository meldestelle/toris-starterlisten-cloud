# -*- coding: utf-8 -*-
# templates/pdf/pdf_spr_mann.py - Mannschaftsprüfungen, sortiert nach Team dann Startnummer
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    style_team = ParagraphStyle('Team', fontSize=10, leading=12, fontName='Helvetica-Bold', alignment=TA_LEFT, textColor=colors.black)
    ...
    # --- Sortierung: teamNumber aufsteigend, dann teamStartingOrder, dann startNumber ---
    def team_sort_key(s):
        team_no = s.get("teamNumber")
        team_ord = s.get("teamStartingOrder")
        start_no = s.get("startNumber")
        # Starter ohne Mannschaft (teamNumber=None) ans Ende
        team_no_key = team_no if team_no is not None else 999999
        team_ord_key = team_ord if team_ord is not None else 999999
        try:
            start_no_key = int(start_no) if start_no is not None else 999999
        except:
            start_no_key = 999999
        return (team_no_key, team_ord_key, start_no_key)

    starters_sorted = sorted(starters, key=team_sort_key)

    # Mannschaftsnamen ermitteln: erster Starter pro teamNumber (nach Sortierung)
    team_name_map = {}     # teamNumber -> team name
    for s in starters_sorted:
        tn = s.get("teamNumber")
        if tn is not None and tn not in team_name_map:
            team_name_map[tn] = s.get("team", "") or ""
    ...
    for s in starters_sorted:
        # Prüfe auf Mannschaftswechsel BEVOR der Starter hinzugefügt wird
        team_no = s.get("teamNumber")

        if team_no != current_team:
            # Neues Team erkannt - Team-Header hinzufügen
            team_name = team_name_map.get(team_no, "") if team_no is not None else ""

            if team_no is not None:
                if team_name:
                    header_text = f"Mannschaft {team_no}: {team_name}"
                else:
                    header_text = f"Mannschaft {team_no}"
            else:
                header_text = "Ohne Mannschaft"

            data_texts.append([header_text, "", "", "", ""])
            meta.append({"type": "team"})
            current_team = team_no
        ...
        if horse:
            ...
            # Besitzer und Züchter entfernt für pdf_spr_kurz
    ...
    # Hilfsfunktion: eine Tabellenzeile aus data_texts/meta bauen
    def build_row(row, m):
        if m["type"] == "header":
            return [
            ...
            return [
                Paragraph(f"<b>{row[0]}</b>", style_team), Paragraph("", style_sub),
    ...
    # --- Globale Header-Tabelle (Spaltenköpfe, wiederholt sich nicht pro Team) ---
    header_row = build_row(data_texts[0], meta[0])
    t_header = Table([header_row], colWidths=col_widths)
    ts_header = base_style()
    ts_header.add("BACKGROUND", (0,0), (-1,0), colors.HexColor('#404040'))
    ts_header.add("TEXTCOLOR", (0,0), (-1,0), colors.white)
    t_header.setStyle(ts_header)
    elements.append(t_header)

    # --- Pauses vor erstem Starter (afterNumberInCompetition=0) direkt anhängen ---
    i = 1
    while i < len(meta) and meta[i].get("type") == "pause":
        pause_row = build_row(data_texts[i], meta[i])
        t_p = Table([pause_row], colWidths=col_widths)
        ts_p = base_style()
        ts_p.add("SPAN", (0,0), (4,0))
        t_p.setStyle(ts_p)
        elements.append(t_p)
        i += 1

    # --- Pro Team eine eigene KeepTogether-Tabelle ---
    # Segmente schneiden: jedes Segment beginnt mit type=="team" (oder Starter ohne Team)
    # Wir iterieren ab i und sammeln zusammengehörige Blöcke
    while i < len(meta):
        m = meta[i]

        if m["type"] == "team":
            # Sammle alle Zeilen dieses Teams (inkl. Pauses)
            seg_rows = []
            seg_meta = []
            seg_rows.append(build_row(data_texts[i], meta[i]))
            seg_meta.append(meta[i])
            i += 1
            while i < len(meta) and meta[i].get("type") != "team":
                seg_rows.append(build_row(data_texts[i], meta[i]))
                seg_meta.append(meta[i])
                i += 1

            # Tabelle für dieses Team bauen
            t_team = Table(seg_rows, colWidths=col_widths)
            ts_team = base_style()
            starter_row_count = 0
            for ri, sm in enumerate(seg_meta):
                if sm["type"] == "team":
                    ts_team.add("SPAN", (0,ri), (4,ri))
                    ts_team.add("BACKGROUND", (0,ri), (4,ri), colors.HexColor('#D0D0D0'))
                    ts_team.add("TOPPADDING", (0,ri), (4,ri), 4)
                    ts_team.add("BOTTOMPADDING", (0,ri), (4,ri), 4)
                    starter_row_count = 0
                elif sm["type"] == "starter":
                    if starter_row_count % 2 == 1:
                        ts_team.add("BACKGROUND", (0,ri), (4,ri), colors.HexColor('#E8E8E8'))
                    starter_row_count += 1
                elif sm["type"] == "pause":
                    ts_team.add("SPAN", (0,ri), (4,ri))
                    ...
                    if not prev_was_gray:
                        ts_team.add("BACKGROUND", (0,ri), (4,ri), colors.HexColor('#E8E8E8'))
                    starter_row_count -= 1
            t_team.setStyle(ts_team)
            elements.append(KeepTogether(t_team))

        else:
            # Starter ohne Mannschaft – einzeln anhängen (kein KeepTogether nötig)
            row_rendered = build_row(data_texts[i], meta[i])
            t_single = Table([row_rendered], colWidths=col_widths)
            ts_single = base_style()
            t_single.setStyle(ts_single)
            elements.append(t_single)
            i += 1
