# -*- coding: utf-8 -*-
# templates/pdf/pdf_dre_pferdewechsel_int.py - Horse exchange format: 3 riders swap horses (English)
#
# Struktur:
#   3 riders ride all 3 horses (9 starts total)
#   Cross table Athlete × Horse with totals
#   Richtverfahren-abhängige Spalten: 402.A / 402.B / 402.C
#
# Pop-up input:  Assignment Athlete ↔ own horse + optional results
# Judges row:    Inline format from pdf_int
# Starting order: Column format from pdf_dre_5_int (full break logic)
...
def get_country_name(ioc_code):
    ...
    names = {
        ...
        "NLD": "Netherlands", "BEL": "Belgium", "FRA": "France",
        "ITA": "Italy", "ESP": "Spain", "PRT": "Portugal",
        "GBR": "Great Britain", "IRL": "Ireland", "SWE": "Sweden",
        "NOR": "Norway", "DNK": "Dänemark", "FIN": "Finland",
        "POL": "Poland", "CZE": "Czech Republic", "HUN": "Hungary",
        "ROU": "Romania", "HRV": "Croatia", "SVN": "Slovenia",
        "GRC": "Griechenland", "TUR": "Turkey", "RUS": "Russia",
        "UKR": "Ukraine", "BLR": "Belarus", "LVA": "Lettland",
        "LTU": "Lithuania", "EST": "Estonia", "USA": "USA",
        "CAN": "Canada", "AUS": "Australien", "NZL": "New Zealand",
        "JPN": "Japan", "KOR": "South Korea", "CHN": "China",
        "BRA": "Brazil", "ARG": "Argentinien", "MEX": "Mexico",
        "RSA": "South Africa", "ZAF": "Südafrika", "ISR": "Israel",
...
# ---------------------------------------------------------------------------
# Build starter content (Athlete + Horse) like pdf_dre_5_int
# ---------------------------------------------------------------------------
def _build_starter_content(s, skip_horse_details=False):
    ...
    if horses:
        ...
        if not skip_horse_details:
            ...
            if bs:
                try:
                    details.append(f"{datetime.now().year - int(bs)}y.")
                except:
                    pass
...
# ---------------------------------------------------------------------------
# Build cross table Athlete × Horse
# ---------------------------------------------------------------------------
def _build_cross_table(starters, own_results, assignment, page_width, judging_rule=""):
    """
    3×3 cross table Athlete × Horse.
    Eigenes-Pferd-Zelle: grau (#c8c8c8), kein Symbol.
    Columns: Label | Horse1 | Horse2 | Horse3 | Σ Athlete
    + Σ Horse row at bottom.
    """
    ...
    # Σ Horse row
    sum_row = [Paragraph("<b>Σ Horse</b>", style_lbl)]
    for _ in range(4):
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    style_pause    = ParagraphStyle("Break",  fontSize=9,  leading=11, fontName="Helvetica-Bold", alignment=TA_CENTER)
    ...
    if judges:
        ...
        if jury_parts:
            judges_para  = Paragraph(f"<b>Judges:</b> {' '.join(jury_parts)}", style_sub)
    ...
    if "402A" in rule:
        ...
        score_labels = ["<b>Task</b>", "<b>Quality</b>", "<b>Total</b>"]
        n_cols       = 3
    else:
    ...
    hdr = [
        "<b># / CNO</b>",
        "<b>Athlete / Horse</b><br/><font size=7>Age / Colour / Sex / Sire x Dam Sire / Owner / Breeder</font>",
    ...
    print(f"PDF Horse Exchange Cloud: {filename}")
