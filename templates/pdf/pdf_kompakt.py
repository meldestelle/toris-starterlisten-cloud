# -*- coding: utf-8 -*-
# templates/pdf/pdf_kompakt.py - Deutsches Design kompakt - mit printOptions-Unterstützung
...
def render(starterlist, filename, logo_max_width_cm=5.0):
    ...
    # Richter als separate Zeile nach Logo-Tabelle
    ...
    col_widths = [12*mm, 22*mm, 16*mm, (page_width - 12*mm - 22*mm - 16*mm - 22*mm) / 2, (page_width - 12*mm - 22*mm - 16*mm - 22*mm) / 2, 22*mm]

    headers = ["Start", "Zeit", "KoNr.", "Reiter", "Pferd", "Ergeb."]
    header_styles = [style_hdr, style_hdr, style_hdr, style_hdr_left, style_hdr_left, style_hdr]
    header_row = [Paragraph(f"<b>{h}</b>", s) for h, s in zip(headers, header_styles)]
    ...
    for starter_index, starter in enumerate(starters):
        ...
        data.append([
            ...
            _maybe_strike(result_text, style_normal, withdrawn)
        ])
